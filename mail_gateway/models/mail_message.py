...
class MailMessage(models.Model):
    ...
    def _to_store(
        ...
        *,
        ...
        for record in self:
            # add_records_fields avoids the recursive _to_store dispatch (O19).
    ...
    def _send_to_gateway_thread(self, gateway_channel_id):
        ...
        notification_domain = [("gateway_channel_id", "=", channel.id)]
        notifications_before = self.env["mail.notification"].search(
            notification_domain, order="id"
        )
        ...
        notification = (
            self.env["mail.notification"].search(notification_domain, order="id")
