...
class MailThread(models.AbstractModel):
    ...
    def _thread_to_store(self, store: Store, fields, *, request_list=None):
        ...
        for record in self:
            # Odoo 19: follower serialization is by-reference (`partner_id`)
            # instead of the old inline `partner` dict, so the previous
            # message_get_followers() parsing no longer works (KeyError 'partner').
            # Compute the gateway-capable follower partners straight from the ORM
            ...
            if partners:
                ...
                store.add_records_fields(
                    record,
                    [{"gateway_followers": [{"id": p.id} for p in partners]}],
                    as_thread=True,
                )
        return res
