...
class ResPartner(models.Model):
    ...
    def _to_store_defaults(self, target):
        # In Odoo 19 the Discuss "Store" no longer exposes ``one_id``; record
        # serialization goes through ``_to_store``/``_to_store_defaults``. We
        # inject the gateway channels the same way the previous ``one_id``
        # monkey-patch did, so follower/persona data keeps ``gateway_channels``.
