...
class MailChannel(models.Model):
    ...
    def _to_store(self, store: Store, fields):
        result = super()._to_store(store, fields)
        if not self:
            return result
        for record in self:
            # add_records_fields adds literal values without re-dispatching to
            # _to_store (which would recurse infinitely in Odoo 19).
